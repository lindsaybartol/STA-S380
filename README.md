...
To dig into this problem, I wanted to start by prepping the data in the same way the "on-staff stats guru" did, so I removed the building that had leasing rates under ten percent. After this, My first instinct was to try and segment the data based on variables that we know for certain regarding the new building. First, it will be a young building, and second, it will have 15 stories.  
<br> Examining the data,  I decided to split it near the quartiles on age and plot the difference in rent prices between green and non-green buildings for buildings under 25 years old, 25-50 years old, 50-75 years old, and 75+ years old. I have utilized bar graphs to focus on median values, as the "stats guru" focused on medians as well.
![Rent Based on Age](rent_age.png)
From the graph, we can see that the difference in rent prices is smaller for newer buildings than it is for older buildings. When we're relying on a small increase in price such as $2.60, these slight differences can make a huge impact. To look at the exact numbers, the median rent for green buildings under 25 years old is $28.10, whereas for non-green buildings, it is $27.23. That's only a $0.87 difference, a third of the size of the original price difference.  
<br> Next, let's take a look at how different numbers of stories can affect the rent difference. In the graphs below, I have segmented the building out into four groups again: 1-5 stories, 5-10 stories, 10-20 stories, and 20+ stories.
...
<br>Now, even though the difference is smaller, one might say that it is still a premium. However, let's examine how these two effects work together. To do so, I am going to reduce the dataframe down to buildings that are more similar to our potential building. I initially filtered the dataframe to only include buildings under 25 years old and with 10-20 stories, but this reduced the dataframe down to under 700 buildings To retain at least 1000 observations, I changed it to buildings under 30 years old and with 10-25 stories. This resulted in a dataframe with almost 1200 observations.  
<br>After filtering the data down to focus on buildings more similar to ours, I plotted a box plot to show the difference in rent prices.
![Rent Filtered](rent_adjusted.png)
Here, we can see that they are very similar, with the green building actually having lower rent prices overall. Here, the median for green buildings is $29.00, and the median for non-green buildings is $30.50. That results in non-green buildings having a $1.50 premium over green buildings!  
<br>Before simply accepting these figures, I wanted to examine at least one more possible confounding figure. I decided to separate out rent differences based on building quality, as I didn't want an uneven mix of building quality to affect the overall trends. The box plots for this can be seen below.
