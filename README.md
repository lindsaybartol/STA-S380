...
To dig into this problem, I wanted to start by prepping the data in the same way the "on-staff stats guru" did, so I removed the buildings that had leasing rates under ten percent. After this, My first instinct was to try and segment the data based on variables that we know for certain regarding the potential new building. First, it will be a young building, and second, it will have 15 stories.  
<br> Examining the data,  I decided to split it near the quartiles on age and plot the differences in rent prices between green and non-green buildings for buildings under 25 years old, 25-50 years old, 50-75 years old, and 75+ years old. I have utilized bar graphs to focus on median values, as the "stats guru" focused on medians as well.
![Rent Based on Age](rent_age.png)
From the graph, we can see that the difference in rent prices is smaller for newer buildings than it is for older buildings. When we're relying on a small increase in price such as $2.60, these slight differences can make a huge impact. Look at the exact numbers, the median rent for green buildings under 25 years old is $28.10, whereas for non-green buildings, it is $27.23. That's only a $0.87 difference, a third of the size of the original price difference.  
<br> Next, let's take a look at how different numbers of stories can affect the rent difference. In the graphs below, I have segmented the buildings out into four groups again: 1-5 stories, 5-10 stories, 10-20 stories, and 20+ stories.
![Rent Based on Stories](rent_stories.png)
Once again, we see that buildings with 10-20 stories have a smaller median difference than buildings with smaller heights. The exact medians here for buildings with 10-20 stories are: $29.00 for green buildings and $27.50 for non-green buildings. Again, we see a reduction in the premium that a green building might give us.  
<br>Now, even though the difference is smaller, one might say that it is still a premium. However, let's examine how these two effects work together. To do so, I am going to reduce the dataframe down to buildings that are more similar to our potential building. I initially filtered the dataframe to only include buildings under 25 years old and with 10-20 stories, but this reduced the dataframe down to under 700 buildings, which is smaller than I would like. To retain at least 1000 observations, I changed it to buildings under 30 years old and with 10-25 stories. This resulted in a dataframe with almost 1200 observations.  
<br>After filtering the data down, I plotted a box plot to show the difference in rent prices.
![Rent Filtered](rent_adjusted.png)
Here, we can see that rents are very similar, but the green buildings actually have lower rent prices overall. The median for green buildings is $29.00, and the median for non-green buildings is $30.50. That results in non-green buildings having a $1.50 premium over green buildings!  
<br>Before simply accepting these figures, I wanted to examine at least one more possible confounding variable. I decided to separate out rent differences based on building quality, as I didn't want an uneven mix of building quality to affect the overall trends. The box plots for this can be seen below.
![Rent Filtered and Based on Classes](rent_adjusted_classes.png)
As we can see, the green buildings in both categories still have lower rent prices, so this does not affect our conclusions.  
<br> Ultimately, based on my analysis, I disagree with the stats guru. There are confounding variables here at play, specifically the age and number of stories. The single box plot on the filtered data shows this. However, while I do not agree with the stats guru, I would still like to do more exploration to confirm my claims. Specifically, I would want to run a regression model to more confidently identify confounders and the effect of green status.  
<br> [Here is my code for this problem.](Green_Buildings.ipynb)
...
<br> To zoom in on the days of the week and confirm my suspicions, I created two side-by-side bar graphs that you'll see below. They measure the average number of people that got on (left) or off (right) the bus on a given day of the week. 
...
<br>As a result of this analysis, I believe that PCA makes more sense as a dimension reduction technique for this dataset, as the overall K-means clustering accuracy for grouping and the accuracy in prediction were both higher than that of tSNE.  
...
## Market Segmentation
To start this problem, I cleaned the data a little bit. I simply removed rows for people who had at least 50% of their tweets categorized as "adult", "spam", or "chatter." This way, we get rid of some bots and some people who did not show strong interests, as they would be difficult to categorize. This brought the number of rows from 7882 to 7855.
...
<br>I decided on this question because I believe it gives an overall interpretation of what the news has been focusing on throughout the time these articles were written. This can show a lot about what the media values and what an audience is interested in hearing during a specific span of time.  
### Approach
...
<br>Ultimately, this gives us insight into what the most prevalent topics are at this time, and what people want to hear about. It succinctly summarizes the issues that are at the forefront of readers' and reporters' minds.  
...
Next, I wanted to explore some rarer combos. I thought this would be important as well because there are some products that are simply bought less frequently, but that doesn't mean that they don't have strong associations. I decided to look at combos with a support between .002 and .005. To try to boost reliability with a lower support, I also raised the confidence and the lift to 0.3 and 3, respectively. This resulted in five associations, shown below.
![Associations Rare](associations_rare.png)
These combinations seem a little more bizarre to me. I am unsure what the link between hamburger meat and instant food products is. However, the lift here is huge. I would like more information on what exactly "instant food products" includes. Popcorn and salty snacks is an interesting combination as well, as I would have thought they'd be substitutes, not complements. The same goes for beer and liquor. Perhaps these are from instances where people were shopping for parties. Rice and vegetables make sense. However, again, I am unsure what the link between turkey and tropical fruit would be. I would guess that they are both bought fairly frequently, and so they end up together at random.  
<br> Overall, we can definitely see some meaningful relationships throughout this data, and I believe that this analysis would give a small grocer a valuable list of items to combine in specials and discounts. The main recommendation I would make would be to first choose the items in the initial list that have a high lift and a relatively larger support. This would include combinations like beef and root vegetables, onions and root vegetables, and berries and whipped cream.  
<br>[Here is the code for this problem.](Association%20-%20Groceries.ipynb)
## Image Classification with Neural Networks
After loading in the data, I built a CNN model to classify the images into one of eleven categories. My model employed two convolution layers and was trained using 25 epochs using the Adam optimizer. I found that fewer epochs resulted in a lower-than-ideal accuracy. 25 epochs seemed to be the sweet spot for reaching a higher accuracy without requiring a large amount of time to run. Below, you can see some of the predictions on the test images.
![image predictions](image_predict.png)
<br> The overall accuracy of my model was 84.37%. This is measured as the total correct predictions out of the total predictions.
